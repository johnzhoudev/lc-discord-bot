...
class ScheduledPost:
    ...
    __should_post_func: Callable[[datetime], bool] # pass datetime.now, will return bool 
    repeats: int = 1

    def __init__(self, get_post_func: Callable[[], Post], should_post_func: Callable[[datetime], bool], repeats: int = 1):
        self.__get_post_func = get_post_func
        self.__should_post_func = should_post_func
        self.repeats = repeats
    
    def get_post(self):
        if self.repeats == 0: return None
        post = self.__get_post_func()
        if self.repeats > 0: self.repeats -= 1 # skip if -1
        return post
...
class DateGenerator:
    ...
    def __init__(self, days: Sequence[int], time: time, called_date: datetime):
        ...
        self.time = time
        self.next_date = called_date.replace(hour=time.hour, minute=time.minute, second=0, microsecond=0)
    ...
    def __call__(self, curtime: datetime) -> bool:
        ...
        # set next date
        self.next_date = self.next_date  + timedelta(days=self.__get_days_to_advance(self.next_date.weekday(), next(self.day_gen)))
